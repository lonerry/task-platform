...
from infrastructure.postgres.repositories import (RefreshTokenRepository,
                                                  UserRepository)
from schemas.auth import CreateRefreshToken, TokenPair, UserWithPassword
...
class LoginUseCase:
    ...
    async def execute(self, email: str, password: str) -> TokenPair:
        async with self._db.session() as session:
            user: UserWithPassword | None = await self._users.get_by_email(
                session, email
            )
            ...
            await self._refresh.create(
                session,
                CreateRefreshToken(
                    user_id=user.id, token=refresh, expires_at=expires_at
                ),
            )
