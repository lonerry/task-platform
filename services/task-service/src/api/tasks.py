from core.exceptions.tasks import (KafkaConnectionError, TaskNotFoundError,
                                   TaskValidationError)
...
async def get_tasks(
    ...
    tasks, total = await use_case.execute(
        current_user.id, limit=request.limit, offset=request.offset
    )
    return Pagination(
        limit=request.limit,
