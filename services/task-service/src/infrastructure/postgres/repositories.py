...
class TaskRepository:
    @log(logger)
    async def create(self, session: AsyncSession, task: CreateTask, user_id: int) -> Task:
    ...
    @log(logger)
    async def get_by_id(self, session: AsyncSession, task_id: int, user_id: int) -> Task | None:
    ...
    @log(logger)
    async def get_all(self, session: AsyncSession, user_id: int, limit: int = 100, offset: int = 0) -> list[Task]:
        query = select(TaskModel).where(TaskModel.user_id == user_id).limit(limit).offset(offset).order_by(TaskModel.created_at.desc())
        result = await session.execute(query)
    ...
    @log(logger)
    async def update(self, session: AsyncSession, task_id: int, task: UpdateTask, user_id: int) -> Task:
        query = update(TaskModel).where(TaskModel.id == task_id, TaskModel.user_id == user_id).values(**task.model_dump()).returning(TaskModel)
        result = await session.execute(query)
        task = result.scalar_one_or_none()
        if task is None:
    ...
    async def delete(self, session: AsyncSession, task_id: int, user_id: int) -> None:
        query = delete(TaskModel).where(TaskModel.id == task_id, TaskModel.user_id == user_id)
        result = await session.execute(query)
