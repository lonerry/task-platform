...
class GetTasksUseCase:
    ...
    async def execute(self, user_id: int, limit: int = 100, offset: int = 0) -> tuple[list[Task], int]:
        async with self.db.session() as session:
            tasks = await self.repo.get_all(session, user_id, limit=limit, offset=offset)
